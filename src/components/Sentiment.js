...
export const Sentiment = (props) => {
	...
	const posts = Posts
	const sadSentimentCount = posts.filter(post => post.props.mood === 0).length
    const happySentimentCount = posts.filter(post =>  post.props.mood === 2).length
	const mehSentimentCount = posts.filter(post => post.props.mood === 1).length
		...
		<div className='flex flex-row mt-5 mb-5'>
			...
			<div className='flex flex-col justify-center items-center'>
				<div className={sentimentState.sentimentTag}>
					<h1 className='flex m-auto justify-center'>{props.mood}</h1>
				</div>
